...
table.insert(connections, RunService.RenderStepped:Connect(function()
    ...
    if Toggles.trigger_enabled.Value and Options.trigger_key:GetState() then
        ...
        if result and result.Instance then
            ...
            if not hum and char and char.Parent and char.Parent:IsA("Model") then
                char = char.Parent
            end
            ...
            if targetPlayer and targetPlayer ~= lp then
                ...
                if Toggles.trigger_teamcheck.Value then
                    if lp.Team and targetPlayer.Team and lp.Team == targetPlayer.Team then isEnemy = false end
                end
            elseif game.PlaceId == 292439477 then
                if hitPart:IsDescendantOf(workspace) then
                    local teamFolder = hitPart
                    for i=1, 5 do
                        if teamFolder.Parent == workspace:FindFirstChild("Players") or not teamFolder.Parent then break end
                        teamFolder = teamFolder.Parent
                    end
                    if teamFolder and teamFolder.Parent == workspace:FindFirstChild("Players") then
                        if not lp.Team or teamFolder.Name ~= lp.Team.Name then
                            isEnemy = true
                        end
                    end
                end
...
local function createEspElements(player)
    ...
    local guiParent = game:GetService("CoreGui"):FindFirstChild("RobloxGui") or workspace
    elements.Highlight.Parent = guiParent
    elements.Highlight.Enabled = false
...
table.insert(connections, RunService.RenderStepped:Connect(function()
    ...
    if Toggles.trigger_enabled.Value and Options.trigger_key:GetState() then
        ...
        if result and result.Instance then
            ...
            local char = hitPart.Parent
            local hum = char:FindFirstChildOfClass("Humanoid")
            if not hum and char.Parent:IsA("Model") then
                char = char.Parent
            end
